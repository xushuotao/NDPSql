...
In today's cloud, storage and compute nodes are typically decoupled on different physical servers, such as Amazon's AWS Cloud~\cite{aws} and Microsoft's Azure Cloud~\cite{azure}.
To perform SQL queries on such clouds, an SQL database system needs to bring flash disk pages of the storage node via the interconnecting network to the CPU cache of the compute node to perform analysis of the fetched data.
With the separation of storage and compute in the cloud, the storage network I/O can become a significant bottleneck for SQL query processing, since the network I/O bandwidth can be much slower than aggregate bandwidth of the flash disks on storage node.

The I/O imbalance of network and flash disks due to the separation of compute and storage can also exist for SQL processing on a single node server.
Like distributed SQL processing, data needs bringing from flash disks via PCIe bus to the CPU cache to perform SQL analysis on a single node.
...
The flash chips inside a flash disk can provide internal aggregate bandwidth that is much greater than the PCIe gen3 bus, if they are organized in a parallel architecture~\cite{biscuit, guz-systor17}.
Therefore, SQL processing on a single node can be also bottlenecked by the PCIe bus bandwidth.

In today's flash storage technology development, industry as well as academia are trying to enable near-storage computational capabilities, such as ARM processors and FPGAs, to application developers~\cite{biscuit, ibex, bluedbm, netezza, exadata, jafar, do2013query, sukhwani2012database, fpga-sort}.
