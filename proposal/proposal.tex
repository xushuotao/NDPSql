...
\title{Near Storage SQL Processing\\ \large MIT CSAIL Research Proposal} \author{Shuotao Xu, Xiangyao Yu\\
...
An SQL database system brings data from flash drive via PCIe bus to CPU cache to perform analysis of the fetched data.

One can improve database system by adding in-storage accelerators in the datapath for processing data-intensive SQL operations. 
Such a bump-in-the-wire accelerator architecture has two advantages: 1) it reduces the amount of disk IOs, and 2) it can fully utilize internal storage bandwidth.
For lots of SQL queries, a large data set is scanned and only a small set of results is produced.
Therefore, disk IOs can be reduced dramatically by offloading SQL operations to the secondary storage.
The second benefit comes from the imbalance between the storage network I/O bandwidth and the SSD IO bandwidth.
With storage technology advancements, such as 3D NAND flash, the internal bandwidth of flash drives can exceed the PCIe and the network bandwidth.
For some advanced flash drives, the internal aggregate bandwidth can be 10X higher than the PCIe bandwidth.
Therefore, by offloading IO-intensive SQL operations, one could boost query performance by fully utilizing all the flash bandwidth.

In this proposal, we focus on using in-storage accelerators to improve SQL query processing by offloading \emph{aggregate} and \emph{join} operations, which are both IO-intensive, to the flash drives.

\begin{itemize}

\item Aggregate Accelerator:
  There are five aggregate functions in SQL: AVG, COUNT,  MIN, MAX and SUM.
  Aggregate functions are often used with GROUPBY operation, where rows are grouped based on column values, and each group of rows is summarized by applying the aggregate functions.
  In order to fully saturate IO, we propose to use a wire-speed accelerator capable of aggregating multiple rows per clock cycle.

\item JOIN Accelerator:
  An SQL JOIN operation combines columns from one or more tables in a relational database.
  A popular implementation of JOIN operation use the hash join algorithm.
  To hash join table A and B, a hash table is first constructed with values of a column in table A.
  Then the values of a column in table B are read to find a match in the hash table.
  In order to reduce the amount of IOs from flash to CPU, we propose an in-storage accelerator in the data path.
  The accelerator 1) constructs an in-storage bloom-filter when data from table A is read, 2) uses the constructed bloom filter to reduce the amount of data from table B needed to be sent to CPU for the hash-join.
\end{itemize}

Other IO-intensive operations, such as filtering, are also beneficial for offloading, but we are not focusing on them since they have been fairly well studied.
