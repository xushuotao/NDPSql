...
To perform SQL queries on such clouds, an SQL database system needs to bring flash disk pages from the storage node via the interconnecting network to the CPU cache of the compute node to perform analysis of the fetched data.
...
The flash chips inside a flash disk can provide a internal aggregate bandwidth that is much greater than the PCIe gen3 bus, if they are organized in a parallel architecture~\cite{biscuit}.
