		...
		if (cand) {						\
          ...
          const bte *bbb = (bte *) Tloc(b, 0); \
			for (r = 0; r < cnt; r++) {			\
                ...
                /* fprintf(stderr, "r = %lu, p = %lu, b[p] = %lx, grps[r] = %lx, concat = %llx, hv = %lx, prb = %lx\n", r, p, (unsigned long int)(bbb[p]), grps[r], (ulng)grps[r]<<8 | (unsigned char)(bbb[p]), hash_lng(hs, (ulng)grps[r]<<8 | (unsigned char)bbb[p]), prb); */ \
				for (hb = HASHget(hs, prb);                             \
				     hb != HASHnil(hs) && hb >= start;	\
				     hb = HASHgetlink(hs, hb)) {	\
					ASSERT;				\
		  ...
		  BAT *b, BAT *s, BAT *g, BAT *e, BAT *h, int subsorted)
{
	...
	if (t == TYPE_str && GDK_ELIMDOUBLES(b->tvheap)) {
      fprintf(stderr, "subgroup using sting offsets\n");
		switch (b->twidth) {
		case 1:
			t = TYPE_bte;
			break;
