...
extern "C"
{
  ...
  uint64_t start, end;
  void init_test1(uint64_t startRow, uint64_t endRow){
    start = startRow;
    end = endRow;
  }
  ...
  inline bool check_result(){
    while ( !result.empty() ){
      ...
      result.pop();
      uint64_t testdata, mydata;
      if ( colBytes == 1 ) {
        mydata = currResult&(0xff);
        testdata = start&(0xff);
      }
      else if ( colBytes == 2){
        mydata = currResult&(0xffff);
        testdata = start&(0xffff);
      }
      else if ( colBytes == 4) {
        mydata = currResult&(0xffffffff);
        testdata = start&(0xffffffff);
      } else if ( colBytes == 8) {
        mydata = currResult;
        testdata = start;
      }
      if ( testdata != mydata || mydata > end ) {
        fprintf(stderr, "check_result wrong: testdata = %lu, mydata = %lu\n", testdata, mydata);
        return false;
      }
      start++;
    }
  ...
  bool inject_rowData(uint32_t* x0){
  ...
  bool inject_rowMask(uint32_t x0){
    rowMaskQ.push(x0);
    produce_result();
    return check_result;
  }
